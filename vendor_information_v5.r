...
write_xlsx(vendor_info, "vendor_info_v5.xlsx")


vendor_info_amz <- vendor_info %>% 
  filter(Vendor == "Amazon") %>% 
  arrange(Description)
#View(vendor_info_amz)

write_xlsx(vendor_info_amz, "vendor_info_amz.xlsx")

vendor_info_amp <- vendor_info %>% 
  filter(Vendor == "Amplify") %>% 
  arrange(Description)
#View(vendor_info_amp)

write_xlsx(vendor_info_amp, "vendor_info_amp.xlsx")

vendor_info_h2m <- vendor_info %>% 
  filter(Vendor == "H2M") %>% 
  arrange(Description)
#View(vendor_info_h2m)

write_xlsx(vendor_info_h2m, "vendor_info_h2m.xlsx")

vendor_info_hyv <- vendor_info %>% 
  filter(Vendor == "HyVee") %>% 
  arrange(Description)
#View(vendor_info_hyv)

write_xlsx(vendor_info_hyv, "vendor_info_hyv.xlsx")

vendor_info_wrd <- vendor_info %>% 
  filter(Vendor == "Ward's") %>% 
  arrange(Description)
#View(vendor_info_wrd)

write_xlsx(vendor_info_wrd, "vendor_info_wrd.xlsx")
